import { checkNativeCurrencyAndTokenBalances } from "utils/utils";
import { useSigningClient } from "../api/cosmwasm";
import { toast } from "react-toastify";
import { COREUM_PAYMENT_COINS, ITEM_ACTION_TYPES, config } from "../api/config";
import axios from "axios";
import { saveItemActivity } from "../api/methods";
import { selectCurrentUser } from "../api/reducers/auth.reducers";
import { changeItemOwnHistory, selectDetailOfAnItem } from "../api/reducers/nft.reducers";
...
export const useCoreumOperations = () => {
  ...
  const { cancelSaleNFT, burnNFT, transferNFT, bidNFT, sendToken, listNFT, acceptSaleNFT }: any = useSigningClient();
